...
uint8_t *SlabAllocator::reallocate_downward(uint8_t *old_p,
    ...
    // inc_ref to prevent old_block from being freed inside reallocate_downward
    // when its internal dec_ref reduces the count, ensuring old_p stays valid
    // for the subsequent memcpy_downward.
...
int ServiceDescriptor::init(const BrpcDescriptorTable& table) {
    ...
    while(pos != strs.npos) {
        ...
        if (!tmp.empty() && tmp != METHOD_SPLIT && tmp != "\n") {
            res.push_back(tmp);
        }
...
bool ParseFbFromIOBUF(Message* msg, size_t msg_size, const butil::IOBuf& buf, size_t meta_size) {
    // buf total size must equal meta_size (RPC header) + msg_size (FlatBuffers payload).
    if (!msg || msg_size == 0 || buf.size() != (msg_size + meta_size)) {
        return false;
    }
    return msg->parse_msg_from_iobuf(buf, msg_size, meta_size);
}
