...
public:
    ...
    void swap(SlabAllocator &other) {
        ...
        std::swap(_full_buf_head, other._full_buf_head);
        std::swap(_fb_begin_head, other._fb_begin_head);
        std::swap(_old_size_param, other._old_size_param);
    }
...
public:
    ...
    // NonreflectableMessage requires MergeFrom to be implemented.
    // FlatBuffers Message is move-only and does not support merging.
    void MergeFrom(const Message& /*from*/) override {
        LOG(FATAL) << "FlatBuffers Message does not support MergeFrom. "
...
struct BrpcDescriptorTable {
    ...
    // Method names separated by EXACTLY ONE space character ' '.
    // e.g. "Method1 Method2 Method3"
    // Tab, newline, or multiple consecutive spaces are NOT supported.
    const std::string method_name_list;
};
...
// Parse a FlatBuffers message from |buf| into |msg|.
// |msg_size|:  byte size of the FlatBuffers payload (excluding meta).
// |meta_size|: byte size of the RPC header / meta prefix in |buf|.
// Requires: buf.size() == msg_size + meta_size.
bool ParseFbFromIOBUF(Message* msg, size_t msg_size, const butil::IOBuf& buf, size_t meta_size = 0);
